...
from . import poll
import projects
from projects import smtk
from projects.smtk.poll import REPO as SMTK_REPO
...
def make_schedulers(buildnames, secrets):
    ...
    codebases = {}
    codebases.update(projects.get_codebase(project=smtk, secrets=secrets))
    codebases.update(projects.get_codebase(poll=poll, secrets=secrets))
    return [
        AnyBranchScheduler(
            name='CMBSuperbuild/SMTK Merge Request Scheduler',
            ...
            codebases=codebases),
        AnyBranchScheduler(
            name='CMBSuperbuild/SMTK Integration Branch Scheduler',
            change_filter=filter.ChangeFilter(
            ...
            codebases=codebases),
        AnyBranchScheduler(
            name='CMBSuperbuild/SMTK SMTK Integration Branch Scheduler',
            change_filter=filter.ChangeFilter(
                category='integration-branch',
                project=SMTK_REPO),
            treeStableTimer=None,
            ...
            codebases=codebases),
        AnyBranchScheduler(
            name='CMBSuperbuild/SMTK SMTK Merge Request Scheduler',
            change_filter=filter.ChangeFilter(
                category='merge-request',
                project=SMTK_REPO,
                # process those SMTK MRs that have the command=superbuild.
                filter_fn=projects.get_change_filter_fn_for_buildbot_commands(accepted_values=['superbuild'])
            ),
            treeStableTimer=None,
            builderNames=buildnames,
            reason='SMTK \'merge-request\' created/changed with \'superbuild\' command',
            codebases=codebases,
            properties={
                'ctest_track': 'buildbot-packages',
            },
